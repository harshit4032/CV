...
def create_mask(img):
    ...
    lower = np.array([0,45,80])
    upper = np.array([144,203,173])
...
while(1):
    ...
    end_row=h
    end_col=w
    frame = frame[start_row:end_row,start_col:end_col] # only considering frame from row 0-300 and col from 300-end so that main focus is on our hands
